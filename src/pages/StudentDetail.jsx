  ...
  if (error || !student) {
    return (
      <div className="flex-1 p-6 flex flex-col items-center justify-center">
        ...
        <h2 className="text-xl font-bold text-on-surface dark:text-white mb-2">{error || "Data not found"}</h2>
    ...
    >
      <div className="flex items-center gap-4 mb-8">
        <button 
          ...
          className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-white/5 transition-colors text-muted hover:text-on-surface dark:hover:text-white border border-gray-200 dark:border-white/10"
        >
          <IconChevronLeft size={24} />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-on-surface dark:text-white tracking-tight">Student Detail</h1>
          <p className="text-sm text-muted">Viewing results for document ID: {document_id}</p>
        </div>
      </div>

      <div className="bg-gray-100/20 dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 rounded-2xl p-6 md:p-8 mb-8 backdrop-blur-xl">
        <div className="flex items-center gap-6 mb-8 pb-8 border-b border-gray-200 dark:border-white/10">
          ...
          <div>
            <h2 className="text-3xl font-bold text-on-surface dark:text-white mb-1">{student.name}</h2>
            <p className="text-muted">{student.email}</p>
          </div>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-black/20 rounded-xl p-6 border border-gray-200 dark:border-white/5 flex flex-col justify-center items-center">
            <div className="text-sm text-muted mb-2 uppercase tracking-wider font-semibold">Total Attempts</div>
            <div className="text-4xl font-bold text-on-surface dark:text-white">{student.total_attempts}</div>
          </div>
          <div className="bg-black/20 rounded-xl p-6 border border-gray-200 dark:border-white/5 flex flex-col justify-center items-center">
      ...
      <div className="bg-gradient-to-r from-primary/10 to-purple-500/10 border border-primary/20 rounded-2xl p-6 md:p-8 backdrop-blur-xl mb-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h3 className="text-xl font-bold text-on-surface dark:text-white mb-4 flex items-center gap-2">
          ...
          {loadingInsights ? (
            ...
            <ul className="space-y-4">
              {insights.map((insight, idx) => (
                <li key={idx} className="flex gap-3 text-on-surface dark:text-white/90">
      ...
      <div className="bg-gray-100/20 dark:bg-white/[0.02] border border-gray-200 dark:border-white/5 rounded-2xl p-6 md:p-8 backdrop-blur-xl mb-8">
        <h3 className="text-xl font-bold text-on-surface dark:text-white mb-6 flex items-center gap-2">
          Test History
        ...
        <div className="space-y-4">
          {student.attempts && student.attempts.length > 0 ? (
            student.attempts.map((attempt, idx) => (
              <div key={idx} className="flex flex-col sm:flex-row sm:items-center justify-between bg-black/20 p-5 rounded-xl border border-gray-200 dark:border-white/5 hover:border-gray-200 dark:border-white/10 transition-colors gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="bg-primary/10 dark:bg-white/10 text-on-surface dark:text-white text-xs font-bold px-2 py-1 rounded">Test {idx + 1}</span>
                    <h4 className="text-on-surface dark:text-white font-semibold text-lg">{attempt.chapter_title}</h4>
                  </div>
                  <div className="text-sm text-muted flex items-center gap-3">
                    <span className="flex items-center gap-1">
                      {new Date(attempt.completed_at).toLocaleString()}
                    </span>
                    <span className="w-1.5 h-1.5 rounded-full bg-gray-200 dark:bg-white/20"></span>
                    <span className="capitalize px-2 py-0.5 rounded text-xs border border-gray-200 dark:border-white/10 bg-gray-100/50 dark:bg-white/5">{attempt.difficulty}</span>
                  </div>
                </div>
                <div className="flex sm:flex-col gap-6 sm:gap-1 items-center sm:items-end sm:text-right border-t sm:border-t-0 border-gray-200 dark:border-white/10 pt-4 sm:pt-0">
                  <div>
                    <div className="text-on-surface dark:text-white font-bold text-2xl leading-none">{attempt.total_score}</div>
                    <div className="text-xs text-muted mt-1">points</div>
                  </div>
                  <div className="w-px h-8 bg-gray-200 dark:bg-white/10 sm:hidden"></div>
                  <div>
                    <div className="text-on-surface dark:text-white font-medium text-lg leading-none">{attempt.time_taken_seconds || 0}</div>
            ...
            <div className="text-center py-12 text-muted border border-dashed border-gray-200 dark:border-white/10 rounded-xl bg-black/10">
